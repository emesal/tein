...
**sandboxing flow**: ContextBuilder with presets → set IS_SANDBOXED thread-local → build full standard env → resolve module allowlist from `Modules` variant via `VFS_REGISTRY` → set VFS gate + allowlist → arm FS policy gate (C-level `tein_fs_policy_gate` + rust thread-local `FS_GATE`) → GC-root source env + null env → create null env (syntax-only) → copy `import` via env_copy_named → register UX stubs for bindings not in the allowlist (each stub looks up providing module in `STUB_MODULE_MAP`) → set null env as active. IS_SANDBOXED + FS_GATE restored to previous value on drop.
