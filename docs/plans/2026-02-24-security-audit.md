...
| 13 | fn pointer transmute not compile-time enforced | resolved | see appendix below |
| 14 | CString inconsistency in error path | resolved | c319731 |
| 15 | unbounded channel in ThreadLocalContext | resolved | ef65526 |

---

## appendix: issue #13 — fn pointer transmute elimination
...
**root cause:** chibi's `sexp_proc1` typedef is `(sexp, sexp, sexp_sint_t) -> sexp` (3-arg), but the variadic calling convention always passes `(ctx, self, nargs, args)` — 4 args, matching chibi's own `sexp_proc2`. the shim `tein_sexp_define_foreign_proc` accepted `sexp_proc1`, forcing rust to `mem::transmute` every fn pointer from 4-arg to 3-arg at every call site.

**fix (3 changes, applied as one logical unit):**

1. **tein_shim.c** — changed `tein_sexp_define_foreign_proc` to accept `sexp_proc2` (chibi's existing 4-arg typedef) instead of `sexp_proc1`. the single `(sexp_proc1)` cast inside the shim is the one intentional bridge between the real ABI and chibi's opcode type.

2. **ffi.rs** — updated both the `extern "C"` declaration and the safe wrapper to use the correct 4-arg fn pointer type `(sexp, sexp, sexp_sint_t, sexp) -> sexp`.

3. **context.rs** — removed all 4 `mem::transmute` sites:
   - `register_protocol_fns` loop (line ~549)
   - IO wrapper registration in `ContextBuilder::build()` (line ~1057)
   - sandbox stub `stub_fn` binding (line ~1091, used by both `ALL_PRESETS` and `ALWAYS_STUB` loops)
   - `define_fn_variadic` public API (line ~1444)

   all extern "C" fn definitions already had the correct 4-arg signature; only the transmutes needed removal.

**key design decision:** used chibi's existing `sexp_proc2` typedef rather than introducing a new `tein_variadic_proc` — avoids maintaining a redundant typedef when chibi already defines the exact type we need.

**verification:** 238 tests pass (205 lib + 12 scheme_fn + 21 doc-tests), clippy clean, `grep -rn "mem::transmute" tein/src/` returns zero matches.
