...
4. **refactor rust side** — rename `ModulePolicy` → `VfsGate`, update builder API, update thread-locals, update ffi wrappers. update `build()` and `drop()`.
   - files to update: `context.rs` (imports, builder fields/methods, `build()`, `drop()`, Context struct fields), `ffi.rs` (rename `module_policy_set` → `vfs_gate_set`, rename+expand callback `tein_module_allowlist_check` → `tein_vfs_gate_check` absorbing VFS prefix/traversal/.scm logic), `value.rs` (thread-local reference), `lib.rs` (re-exports if any)
   - the new rust callback `tein_vfs_gate_check` must absorb: VFS `/vfs/lib/` prefix check, `..` path traversal guard, `.scm` passthrough, allowlist prefix matching (all previously in C)
5. **update tests** — all existing sandbox/module tests, add dep-resolution unit tests.
