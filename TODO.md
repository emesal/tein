...
- [ ] **parameterised IO presets**
  - thread-local `FsPolicy` with path prefix matching
  - wrapped foreign functions checking policy before delegating
  - `Preset::FileRead(&["/config/"])`, `Preset::FileWrite(&["/tmp/"])` style api
